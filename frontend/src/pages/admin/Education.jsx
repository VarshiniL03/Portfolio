...
export default function AdminEducation() {
  ...
  async function load() {
    const res = await listEducation(true);
    setItems(res.data);
  }
  ...
  return (
    <AdminLayout>
      <h1 className="font-display text-2xl mb-6">Education</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-ink-alt border border-ink-border rounded-xl p-6 mb-8 grid md:grid-cols-2 gap-4">
        ...
        />
        <input
          placeholder="Grade (e.g. 8.7 CGPA)"
          value={form.grade || ""}
      ...
      <div className="space-y-3">
        {items.map((item) => (
          <div key={item.id} className="flex justify-between items-center bg-ink-alt border border-ink-border rounded-lg p-4">
            <div>
              <p className="font-display">{item.degree} · {item.institution}</p>
